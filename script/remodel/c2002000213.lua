...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Add 1 "Worm" or "W Nebula" card from your Deck to your hand, except "Worm Illidan"
...
function s.wormfilter(c,tp)
	return (c:IsSetCard(SET_WORM) and c:IsRace(RACE_REPTILE))
		and c:IsControler(tp)
end
function s.spcon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(s.wormfilter,1,nil,tp)
end
...
function s.thfilter(c)
	return (c:IsSetCard(SET_WORM) or c:IsSetWNebula()) 
		and c:IsAbleToHand() and not c:IsCode({id,57543573})
end
