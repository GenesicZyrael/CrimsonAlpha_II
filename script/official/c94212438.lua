...
function s.plop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.IsPlayerAffectedByEffect(tp,2002000244) and Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsStatus,STATUS_SPSUMMON_TURN),tp,0,LOCATION_MZONE,1,nil) 
	and Duel.SelectYesNo(tp,aux.Stringid(2002000244,2)) then
		ct=Duel.GetMatchingGroupCount(aux.FaceupFilter(Card.IsStatus,STATUS_SPSUMMON_TURN),tp,0,LOCATION_MZONE,nil)+1
	end
	...
	if #sg>0 then
		for i=1,ct do
			Duel.MoveToField(list[i],tp,tp,LOCATION_SZONE,POS_FACEUP,true) 
