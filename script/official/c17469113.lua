...
function s.initial_effect(c)
	...
	e0:SetValue(SUMMON_TYPE_SPECIAL+1) -- Assign a specific Summon Type value
	c:RegisterEffect(e0)
	...
	--Register flag when Special Summoned by its own procedure
	local e5=Effect.CreateEffect(c)
	e5:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_CONTINUOUS)
	e5:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	e5:SetCode(EVENT_SPSUMMON_SUCCESS)
	e5:SetCondition(s.flagcon)
...
-- ==========================================
-- Flag Registration Logic
-- ==========================================
function s.flagcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsSummonType(SUMMON_TYPE_SPECIAL+1)
end
function s.flagop(e,tp,eg,ep,ev,re,r,rp)
	-- Registers the flag using Avida's ID (17469113) for the whole duel (no reset parameters)
